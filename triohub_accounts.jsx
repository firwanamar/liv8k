...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        let data = null;
        try { const r = await window.storage.get(REC_KEY); data = r ? JSON.parse(r.value) : null; } catch (e) {}
        if (!(Array.isArray(data) && data.length)) {
          try { const o = await window.storage.get(OLD_KEY); const od = o ? JSON.parse(o.value) : null; if (Array.isArray(od) && od.length) data = od; } catch (e) {}
        }
      ...
      try { const p = await window.storage.get(PARTNERS_KEY); const pd = p ? JSON.parse(p.value) : null; if (Array.isArray(pd) && pd.length) setPartners(pd); } catch (e) {}
    })();
  }, []);
  ...
  useEffect(() => {
    ...
    setSaved(false);
    (async () => { try { await window.storage.set(REC_KEY, JSON.stringify(records)); setSaved(true); } catch (e) {} })();
  }, [records]);

  useEffect(() => {
    if (partFirst.current) { partFirst.current = false; return; }
    (async () => { try { await window.storage.set(PARTNERS_KEY, JSON.stringify(partners)); } catch (e) {} })();
  }, [partners]);
